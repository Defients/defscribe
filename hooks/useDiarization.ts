...
function processFrame(frame, t0ms) {
  // Defensive: ensure Float32Array and length > 0
  if (!frame || !frame.length) return;

  // Extract features directly (no WebAudio nodes in worker)
  const features = meyda.extract(['rms', 'mfcc'], frame, {
  ...
  // Adaptive noise floor (slowly moves toward observed rms)
  noiseFloor = (noiseFloor * 0.995) + (rms * 0.005);
  const vadThreshold = Math.max(noiseFloor * 2.5, 0.0045); // minimum floor
  ...
  if (centroids.length === 0) {
    ...
    if (minDistance > CHANGE_THRESHOLD && centroids.length < cfg.expectedSpeakers) {
      ...
      // mild centroid update to reduce drift (EMA)
...
self.onmessage = (e) => {
  ...
  try {
    if (type === 'config') {
      ...
      // payload: { frame: Float32Array, t0ms: number }
...
const useDiarization = (
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      // Ensure the audio context is running, which is required before loading modules.
      ...
      // No need to connect worklet to destination as it's only for analysis.
      ...
      worker.onmessage = (e: MessageEvent) => {
        const { type, payload } = e.data || {};
        if (type === 'segments' && Array.isArray(payload)) {
